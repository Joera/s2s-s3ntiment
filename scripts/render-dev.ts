...
function loadSpecs(): RenderSpec[] {
  ...
  // s2s-s3ntiment's home template iterates five collections (why,
  // how-respondent, how-organisator, features, who) — mirror mapping.json's
  // templateConfig.collections so the shared pure filter populates them from
  // allDeals[].
  const homeCollections = [
    { slug: 'why', filters: [{ postType: 'why' }] },
    { slug: 'how-respondent', filters: [{ postType: 'how', tags: 'respondent' }] },
    { slug: 'how-organisator', filters: [{ postType: 'how', tags: 'organisator' }] },
    { slug: 'features', filters: [{ postType: 'feature' }] },
    { slug: 'who', filters: [{ postType: 'who' }] },
  ];
  ...
  return [
    ...
    {
      // why.handlebars renders a raw {{{content}}} sink (no extract_images
      // mangling), so it's the crisp place to prove the sections content
      // transform + injected helpers end-to-end. Its collections.why block is
      // driven by the same allDeals[] + templateConfig.collections filter.
      name: 'why',
      templateFile: 'why',
      body: body('why'),
      templateConfig: {
        ...
        file: 'why',
        path: '/{slug}/',
        collections: [{ slug: 'why', filters: [{ postType: 'why' }] }],
      },
      allDeals,
    },
  ];
...
async function loadContractSpecs(limit?: number): Promise<RenderSpec[]> {
  ...
  // Home template iterates five collections, driven by the pulled allDeals[]
  // through the shared filter (mirrors mapping.json's templateConfig).
